...
pred_file_id = fopen('result.bin', 'r');
% getting GT file
if strcmp(dataset_name, '7-Scenes')
    file_id = fopen('/mnt/lustre/dingmingyu/Research/ICCV19/NNnet/data/sevenscenes/log_near_5/neardata5.txt')
%    file_id = fopen('/mnt/lustre/dingmingyu/Research/ICCV19/camera-relocalisation/cnn_part/data/NN_7scenes_bak.txt')
...
orientation_est = estimations(:, 4:end);
translation_est = estimations(:, 1:3);
...
for k=1:number_of_pairs
    ...
    if NN_count == 5
        ...
        if numel(find(isnan(trans_tmp))) == 10
            ...
            if init_estimate_inlier_T > 0 && numel(sim_inlier_cnt_T) > 1 %if exists such other estimate
