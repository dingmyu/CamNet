...
def get_parser():
    ...
    parser.add_argument('--batch_size', type=int, default=64*6, help='batch size')
...
def validate(val_loader, data_list, model, model_pfr, model_prp):
    ...
    import struct
    fpfr = open('result_pfr.bin', 'wb')
    fprp = open('result_prp.bin', 'wb')
    for i, (input1, input2, translation, quaternions) in enumerate(val_loader):
        ...
        trans, quat = model_prp(torch.cat([x1_PRP,x2_PRP], 1))
